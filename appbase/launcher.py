...
class Launcher(QtGui.QMainWindow):
	...
	def __init__(self,
		...
		#rootFrame.setContentsMargins(0, 0, 0, 0)
		rootFrame.setFrameStyle(QtGui.QFrame.Panel | QtGui.QFrame.Sunken)#QtGui.QFrame.Raised)
		rootFrame.setLineWidth(2)
		...
		self.fileSystemModel = _FileSystemModel(self.treeView)
		self.fileSystemModel.setNameFilters(['*.pyz'])
	...
	def _changeRootDir(self):
		path =dialogs.getExistingDirectory(directory=self._path)
		if path:
...
class _FileSystemModel(QtGui.QFileSystemModel):
	...
	def isPyz(self,index):
		return unicode(self.fileName(index)).endswith('.pyz')
